...
macro_rules! eprintln { ($($arg:tt)*) => { if !tex_kpse::fs::is_memory() { std::eprintln!($($arg)*); } }; }
...
fn parse_aux_file(path: &Path, aux: &mut Aux, depth: usize) -> Result<(), String> {
    ...
    let src = tex_kpse::fs::read_to_string(path)
...
fn read_text_file(path: PathBuf) -> Result<ResolvedText, String> {
    ...
    tex_kpse::fs::read_to_string(&path)
...
fn resolve_text_with_kpse(
    ...
    match kpse.find(name, fmt) {
        Some(hit) if hit.tex_is_file() => return read_text_file(hit).map(Some),
        _ => {}
    }
...
pub fn run(args: &[String], version: &str) -> i32 {
    ...
    if let Err(e) = tex_kpse::fs::write(&bbl_path, &bbl) {
...
fn run_cmd(
    ...
    match cmd {
        BstCmd::Entry { fields, ints, strs } => {
            if *read_done {
                return Err("ENTRY command after READ".into());
            }
