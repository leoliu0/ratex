...
impl FontResolver for Engine {
    fn char_width(&self, f: FontId, c: u8) -> i32 {
        self.eqtb.fonts.get(f as usize).map(|f| f.char_width(c)).unwrap_or(0)
    }
    fn char_height(&self, f: FontId, c: u8) -> i32 {
        self.eqtb.fonts.get(f as usize).map(|f| f.char_height(c)).unwrap_or(0)
    }
    fn char_depth(&self, f: FontId, c: u8) -> i32 {
        self.eqtb.fonts.get(f as usize).map(|f| f.char_depth(c)).unwrap_or(0)
    }
    fn char_italic(&self, f: FontId, c: u8) -> i32 {
        self.eqtb.fonts.get(f as usize).map(|f| f.char_italic(c)).unwrap_or(0)
    }
}

impl Engine {
    /// public wrapper for expansion dispatch from active chars
    pub fn expand_prim_pub(&mut self, p: Prim, id: crate::token::CsId) -> Option<crate::token::Token> {
