...
pub fn fix_jump(fs: &mut FuncState, pc: usize, target: usize) {
    ...
    let offset = (target as isize) - (pc as isize) - 1;
    let max_sj = if fs.lexer.level == crate::LuaLanguageLevel::Lua53 {
        (1 << 17) - 1
    } else {
        (Instruction::MAX_SJ >> 1) as isize
    };
    if offset < -max_sj || offset > max_sj {
        if fs.checklimit_error.is_none() {
            fs.checklimit_error = Some(fs.sem_error("control structure too long"));
        }
        return;
    }
    let instr = &mut fs.chunk.code[pc];
...
fn constfolding(_fs: &FuncState, op: BinaryOperator, e1: &mut ExpDesc, e2: &ExpDesc) -> bool {
    ...
    match op {
        ...
        OpAdd | OpSub | OpMul | OpIDiv | OpMod => {
            // Lua integer arithmetic is defined modulo 2^64. Constant folding
            // must preserve that contract instead of converting overflow to a
            // floating-point result.
            if e1_is_int_type && e2_is_int_type {
                let result = match op {
                    OpAdd => i1.wrapping_add(i2),
                    OpSub => i1.wrapping_sub(i2),
                    OpMul => i1.wrapping_mul(i2),
                    OpIDiv => {
                        if i2 == -1 {
                            i1.wrapping_neg()
                        } else {
