...
use crate::compiler::parse_literal::{
    NumberResult, decode_byte_source_markers, parse_float_token_value, parse_int_token_value,
    parse_string_token_value,
};
use crate::compiler::parser::{
    BinaryOperator, LuaLanguageLevel, LuaTokenKind, UNARY_PRIORITY, UnaryOperator,
...
fn simpleexp(fs: &mut FuncState, v: &mut ExpDesc) -> Result<(), String> {
    match fs.lexer.current_token() {
        ...
        LuaTokenKind::TkString | LuaTokenKind::TkLongString => {
            ...
            let text = fs.lexer.current_token_text();
            let kind = fs.lexer.current_token();
            let has_byte_markers = fs.lexer.has_byte_markers();
            let string_content = parse_string_token_value(text, kind).map(|bytes| {
                if has_byte_markers {
                    decode_byte_source_markers(bytes)
                } else {
                    bytes
                }
            });
...
fn funcargs(fs: &mut FuncState, f: &mut ExpDesc, line: usize) -> Result<(), String> {
    ...
    match fs.lexer.current_token() {
        ...
        LuaTokenKind::TkString | LuaTokenKind::TkLongString => {
            ...
            let kind = fs.lexer.current_token();
            let has_byte_markers = fs.lexer.has_byte_markers();
            let mut vec8 = parse_string_token_value(text, kind)?;
            if has_byte_markers {
...
pub fn body(fs: &mut FuncState, v: &mut ExpDesc, is_method: bool) -> Result<(), String> {
    ...
    child_fs.check_pending_checklimit()?;
