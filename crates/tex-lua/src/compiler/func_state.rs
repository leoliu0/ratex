...
impl<'a> FuncState<'a> {
    ...
    pub fn enter_level(&mut self) -> Result<(), String> {
        ...
        if self.lexer.nesting_level >= MAXCCALLS {
            let message = if self.lexer.level == crate::LuaLanguageLevel::Lua53 {
                "too many C levels"
            } else {
    ...
    pub fn token_error(&self, msg: &str) -> String {
        ...
        if token_text == "<eof>" {
            ...
            if self.lexer.has_byte_markers()
                && token_text.starts_with(BYTE_SOURCE_MARKER)
                && token_text.len() == BYTE_SOURCE_MARKER.len_utf8()
            {
                let range = self.lexer.current_token_range();
                if let Some(payload) = self.lexer.origin_text()[range.end_offset()..]
                    .chars()
                    .next()
                {
