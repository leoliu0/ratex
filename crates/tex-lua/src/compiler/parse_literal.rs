...
use crate::compiler::{BYTE_SOURCE_BASE, BYTE_SOURCE_ESCAPE, BYTE_SOURCE_MARKER};

pub(crate) fn decode_byte_source_markers(bytes: Vec<u8>) -> Vec<u8> {
    let marker = BYTE_SOURCE_MARKER.to_string().into_bytes();
    let escape = BYTE_SOURCE_ESCAPE.to_string().into_bytes();
    let mut output = Vec::with_capacity(bytes.len());
    let mut offset = 0;
    while offset < bytes.len() {
        if !bytes[offset..].starts_with(&marker) {
            output.push(bytes[offset]);
            offset += 1;
            continue;
        }
        let payload = offset + marker.len();
        if bytes[payload..].starts_with(&escape) {
            output.extend_from_slice(&marker);
            offset = payload + escape.len();
            continue;
        }
        if let Ok(text) = std::str::from_utf8(&bytes[payload..])
            && let Some(ch) = text.chars().next()
        {
            let value = u32::from(ch);
            if (BYTE_SOURCE_BASE..BYTE_SOURCE_BASE + 256).contains(&value) {
                output.push((value - BYTE_SOURCE_BASE) as u8);
                offset = payload + ch.len_utf8();
                continue;
            }
        }
        output.extend_from_slice(&marker);
