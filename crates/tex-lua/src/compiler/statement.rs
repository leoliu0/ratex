...
use crate::compiler::parser::{LuaLanguageLevel, LuaTokenKind};
    ...
    match fs.lexer.current_token() {
        ...
        _ => {
            ...
            if fs.lexer.level == LuaLanguageLevel::Lua55
                && fs.lexer.current_token_text() == "global"
            {
...
fn fornum(fs: &mut FuncState, varname: String, line: usize) -> Result<(), String> {
    ...
    if fs.lexer.level == LuaLanguageLevel::Lua53 {
        return fornum53(fs, varname, line);
    }
    ...
    // Lua 5.3 loop variables remain ordinary assignable locals.
    let loop_var_kind = if fs.lexer.level == LuaLanguageLevel::Lua55 {
        VarKind::RDKCONST
    } else {
        VarKind::VDKREG
    };
    fs.new_localvar(varname, loop_var_kind);
...
fn fornum53(fs: &mut FuncState, varname: String, line: usize) -> Result<(), String> {
    let base = fs.freereg;
    fs.new_localvar("(for index)".to_string(), VarKind::VDKREG);
    fs.new_localvar("(for limit)".to_string(), VarKind::VDKREG);
    fs.new_localvar("(for step)".to_string(), VarKind::VDKREG);
    fs.new_localvar(varname, VarKind::VDKREG);

    check(fs, LuaTokenKind::TkAssign)?;
    fs.lexer.bump();
    let mut initial = expr(fs)?;
    code::exp2nextreg(fs, &mut initial);
    check(fs, LuaTokenKind::TkComma)?;
    fs.lexer.bump();
    let mut limit = expr(fs)?;
    code::exp2nextreg(fs, &mut limit);
    if testnext(fs, LuaTokenKind::TkComma) {
        let mut step = expr(fs)?;
        code::exp2nextreg(fs, &mut step);
    } else {
        code::code_asbx(fs, OpCode::LoadI, fs.freereg as u32, 1);
        code::reserve_regs(fs, 1);
    }
    ...
    let prep_pc = code::code_abx(fs, OpCode::ForPrep53, base as u32, 0);
    code::fixline(fs, line);
    check(fs, LuaTokenKind::TkDo)?;
    fs.lexer.bump();

    let block_id = fs.compiler_state.alloc_blockcnt(BlockCnt {
        previous: None,
        first_label: 0,
        first_goto: 0,
        nactvar: fs.nactvar,
    ...
    enterblock(fs, block_id, 0);
    fs.adjust_local_vars(1)?;
    code::reserve_regs(fs, 1);
    block(fs)?;
    leaveblock(fs)?;

    let loop_pc = code::code_abx(fs, OpCode::ForLoop53, base as u32, 0);
    fix_for_jump(fs, prep_pc, loop_pc, false)?;
    fix_for_jump(fs, loop_pc, prep_pc + 1, true)?;
    code::fixline(fs, line);
    Ok(())
...
fn forlist(fs: &mut FuncState, indexname: String) -> Result<(), String> {
    ...
    fs.new_localvar(indexname, loop_var_kind);
    ...
    let lua53 = fs.lexer.level == LuaLanguageLevel::Lua53;
    adjust_assign(fs, if lua53 { 3 } else { 4 }, nexps, &mut e);
    ...
    let prep_pc = if lua53 {
        code::jump(fs)
    } else {
        let pc = code::code_abx(fs, OpCode::TForPrep, base as u32, 0);
        fs.freereg -= 1;
        pc
    };
    ...
    if lua53 {
        code::fix_jump(fs, prep_pc, fs.pc);
        code::code_abc(fs, OpCode::TForCall53, base as u32, 0, (nvars - 3) as u32);
        code::fixline(fs, line);
        let loop_pc = code::code_abx(fs, OpCode::TForLoop53, base as u32, 0);
        fix_for_jump(fs, loop_pc, prep_pc + 1, true)?;
        code::fixline(fs, line);
    } else {
        ...
        code::fixline(fs, line);
        let loop_pc = code::code_abx(fs, OpCode::TForLoop, base as u32, 0);
        fix_for_jump(fs, loop_pc, prep_pc + 1, true)?;
        code::fixline(fs, line);
    }
