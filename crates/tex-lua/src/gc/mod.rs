...
pub struct GC {
    ...
    /// Queue order preserves interrupted batches ahead of objects discovered
    /// by a later collection.
    tobefnz: VecDeque<GcObjectPtr>,
...
impl GC {
    ...
    /// Execute a Lua C API `LUA_GCSTEP` request. Lua 5.3 expresses the
    /// requested work in KiB, while the collector's native step uses its
    /// configured byte-sized quantum.
    pub(crate) fn api_step(&mut self, l: &mut LuaState, kibibytes: i32) -> bool {
        let old_stopped = self.gc_stopped;
        self.gc_stopped = false;
        let iterations = if kibibytes <= 0 {
            1
        } else {
            let bytes = (kibibytes as usize).saturating_mul(1024);
            let quantum = self.apply_param(STEPSIZE, 100).max(1) as usize;
            bytes.saturating_add(quantum - 1) / quantum
        };
    ...
    fn single_step(&mut self, l: &mut LuaState, fast: bool) -> StepResult {
        ...
        let result = match self.gc_state {
            ...
            GcState::CallFin => {
                ...
                if !self.tobefnz.is_empty() && !self.gc_emergency && self.finalizer_error.is_none()
                {
                    self.call_one_finalizer(l);
                    if self.finalizer_error.is_some() {
    ...
    fn atomic2gen(&mut self, l: &mut LuaState) {
        ...
        self.tobefnz = self.sweep2old_ptr_list(tobefnz.into(), other_white).into();
    ...
    fn atomic(&mut self, l: &mut LuaState) {
        ...
        // Lua 5.3 keeps a weak closure cache per prototype. Remove entries
        // whose prototype or closure was not reached before sweep frees them.
        l.global_state_mut()
            .closure_cache53
            .retain(|proto, function| {
                !proto.as_ref().header.is_white() && !function.as_ref().header.is_white()
            });
    ...
    fn sweep_full_list(
        ...
        tobefnz: &mut VecDeque<GcObjectPtr>,
        l: &mut LuaState,
        other_white: u8,
        current_white: u8,
    ) -> Vec<GcObjectOwner> {
    ...
    fn udata2finalize(&mut self) -> Option<GcObjectPtr> {
        ...
        // Store finalizers in call order so an interrupted batch stays ahead
        // of objects separated during the next collection.
        let gc_ptr = self.tobefnz.pop_front()?;
    ...
    fn call_one_finalizer(&mut self, l: &mut LuaState) {
        ...
        // Lua 5.3 only invokes callable __gc values.
        let Some(gc_method) =
            get_metamethod_event(l, &obj_value, TmKind::Gc).filter(LuaValue::is_function)
        else {
            return;
        };
        ...
        let failure = match result {
            Ok((true, _)) => None,
            Ok((false, values)) => Some(
                values
                    .first()
                    .and_then(LuaValue::as_bytes)
                    .map(|bytes| String::from_utf8_lossy(bytes).into_owned())
                    .unwrap_or_else(|| "error object is not a string".to_string()),
            ),
            Err(error) => Some(l.get_error_msg(error)),
        };
    ...
    fn call_all_pending_finalizers(&mut self, l: &mut LuaState) {
        while !self.tobefnz.is_empty() && !self.gc_emergency && self.finalizer_error.is_none() {
            self.call_one_finalizer(l);
        }
    }

    pub(crate) fn finalize_all_on_close(&mut self, state: &mut LuaState) {
        // Match luaC_freeallobjects: separate the current finalizable set once.
        // A finalizer may re-register or create objects, but shutdown must not
        // repeatedly finalize that newly queued set.
        self.separate_to_be_finalized(true);
        self.call_all_pending_finalizers(state);
    }
    ...
    fn separate_to_be_finalized(&mut self, all: bool) {
        ...
        while i < self.finobj.len() {
            ...
            let is_white = gc_ptr
                ...
                .unwrap_or(false);
            if is_white || all {
                selected.push(self.finobj.remove(i));
            } else {
                i += 1;
            }
        }
        selected.reverse();
        self.tobefnz.extend(selected);
    }
    ...
    pub(crate) fn take_finalizer_error(&mut self) -> Option<String> {
        self.finalizer_error.take()
    }
