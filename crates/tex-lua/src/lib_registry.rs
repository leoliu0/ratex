...
fn entry_supported(language: LuaLanguageLevel, module: &str, entry: &str) -> bool {
    const LUA53_MATH_COMPAT: &[&str] = &["atan2", "cosh", "log10", "pow", "sinh", "tanh"];
    if language != LuaLanguageLevel::Lua53 {
        return !(module == "math" && LUA53_MATH_COMPAT.contains(&entry));
    }
    !matches!(
        (module, entry),
        ("_G", "warn") | ("coroutine", "close") | ("table", "create")
    )
}
...
/// Create a standard-library registry; state-specific version filtering occurs
/// when the registry is installed.
pub fn create_standard_registry(open_lib: Stdlib) -> LibraryRegistry {
    let mut registry = LibraryRegistry::new();

    if matches!(open_lib, Stdlib::All | Stdlib::Bit32) {
        registry.register(stdlib::bit32::create_bit32_lib());
    }
