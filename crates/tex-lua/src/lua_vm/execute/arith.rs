...
pub(crate) fn op_bitwise(
    ...
    let mut i1 = 0i64;
    let mut i2 = 0i64;
    if ptointeger(base_stk.offset(b).get_ref(), &mut i1, coerce_strings)
        && ptointeger(base_stk.offset(c).get_ref(), &mut i2, coerce_strings)
    {
        *pc += 1;
...
#[inline]
pub fn ptointeger(v: &LuaValue, out: &mut i64, coerce_strings: bool) -> bool {
    if v.ttisinteger() {
        *out = v.ivalue();
        return true;
    }
    if v.ttisfloat() {
        let number = v.fltvalue();
    ...
    if coerce_strings && let Some(text) = v.as_str() {
        let number = crate::stdlib::basic::parse_number::parse_lua_number(text);
        return ptointeger(&number, out, false);
    }
    false
}
