...
pub fn call_c_function(
    ...
    // Only run GC safe point if results are within the caller's frame.
    // In metamethod calls (where func_idx >= frame_top), the result is above
    // frame_top and will be moved into dest by the metamethod caller.
    if func_idx < frame_top {
        lua_state.check_gc_safe_point();
    }
    Ok(())
}
