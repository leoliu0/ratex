...
use crate::{
    ...
    lua_value::LuaValue,
    lua_vm::{
        CallInfo, LuaError, LuaResult, LuaState,
        execute::{
    ...
    loop {
        ...
        if let Some(nn) = concat_utf8_run(lua_state, top, total)? {
            ...
            if (!v1.ttisstring() && !cvt2str(v1)) || !tostring_inplace(lua_state, top - 1)? {
                ...
                match tryconcattm(lua_state, top) {
                    Ok(()) => {}
                    Err(LuaError::Yield) => {
                        if let Some(ci) = ci.as_deref_mut() {
                            ci.set_pending_finish_get(top as i32);
                        }
