        ...
        loop {
            ...
            match instr.get_opcode() {
                ...
                OpCode::Concat => {
                    ...
                    // ProtectNT
                    ci.save_pc(pc);
                    match concat(lua_state, Some(ci), n as usize) {
                        Ok(()) => {}
                        Err(LuaError::Yield) => {
                ...
                OpCode::ForPrep53 => {
                    let a = instr.get_a() as usize;
                    let initial_slot = base_stk.offset(a);
                    let limit_slot = initial_slot.offset(1);
                    ...
                    let mut limit = 0.0;
                    let mut step = 0.0;
                    if !for_tonumber(initial_slot, &mut initial) {
                        return Err(
                            lua_state.error("'for' initial value must be a number".to_string())
                        );
                    }
                    if !for_tonumber(limit_slot, &mut limit) {
                        return Err(lua_state.error("'for' limit must be a number".to_string()));
                    }
                    if !for_tonumber(step_slot, &mut step) {
                        return Err(lua_state.error("'for' step must be a number".to_string()));
                    }
                    initial_slot.set_float(initial - step);
                    limit_slot.set_float(limit);
                    step_slot.set_float(step);
                    pc += instr.get_bx() as usize;
                    updatetrap!();
                }
                OpCode::ForLoop53 => {
                    let a = instr.get_a() as usize;
                    let index_slot = base_stk.offset(a);
                    let limit_slot = index_slot.offset(1);
                    let step_slot = index_slot.offset(2);
                    let continue_loop = if index_slot.is_integer()
                        && limit_slot.is_integer()
                        && step_slot.is_integer()
                    {
                        let step = step_slot.ivalue();
                        let index = index_slot.ivalue().wrapping_add(step);
                        let limit = limit_slot.ivalue();
                        let keep_going = if step > 0 {
                            index <= limit
                        } else {
                            limit <= index
                        };
                        if keep_going {
                            index_slot.set_integer(index);
                            index_slot.offset(3).set_integer(index);
                        }
                        keep_going
                    } else {
                        let index = index_slot.fltvalue() + step_slot.fltvalue();
                        let step = step_slot.fltvalue();
                        let limit = limit_slot.fltvalue();
                        ...
                        if keep_going {
                            index_slot.set_float(index);
                            index_slot.offset(3).set_float(index);
                        }
                        keep_going
                    };
                    if continue_loop {
                ...
                OpCode::TForCall53 => {
                    let a = instr.get_a() as usize;
                    let result_count = instr.get_c() as usize;
                    let function_index = ci.base + a + 3;
                    base_stk.offset(a + 5).set(base_stk.offset(a + 2));
                    base_stk.offset(a + 4).set(base_stk.offset(a + 1));
                    base_stk.offset(a + 3).set(base_stk.offset(a));
                    lua_state.set_top_raw(function_index + 3);
                    ci.save_pc(pc);
                    if precall(lua_state, function_index, 2, result_count as i32)? {
                        reload_after_call!();
                        continue;
                    }
                    if lua_state.hook_mask & LUA_MASKLINE != 0 {
                ...
                OpCode::TForLoop53 => {
                    let a = instr.get_a() as usize;
                    let next = base_stk.offset(a + 3).get();
                    if !next.is_nil() {
                        base_stk.offset(a + 2).write(&next);
                        pc -= instr.get_bx() as usize;
                    }
                }
                OpCode::ForLoop => {
