...
fn finishget_core(
    ...
    for _ in 0..MAXTAGLOOP {
        let tm = if let Some(table) = t.as_table_mut() {
            ...
            let fast = mt.impl_table.get_shortstr_fast(event_key);
            match fast {
                Some(v) => v,
                None => {
...
fn finish_c_api_continuation(lua_state: &mut LuaState, ci: &mut CallInfo) -> LuaResult<()> {
    ...
    let function = ci.c_k_function;
    let context = ci.c_k_context;
    ...
    let available = stack_top.saturating_sub(ci.base);
    if result_count > available {
        return Err(lua_state.error("C continuation returned too many results".to_string()));
    }
    let first_result = stack_top - result_count;

    lua_state.pop_frame();
    if result_count != 0 && function_pos != first_result {
        lua_state
            .stack_mut()
            .copy_within(first_result..first_result + result_count, function_pos);
    }
    let final_count = if nresults < 0 {
        result_count
    } else {
        let wanted = nresults as usize;
        for index in result_count..wanted {
            lua_state.stack_set(function_pos + index, LuaValue::nil())?;
        }
        wanted
    };
    let new_top = function_pos + final_count;
    lua_state.set_top_raw(new_top);
    if lua_state.call_depth() > 0 {
        let caller = lua_state.call_depth() - 1;
        if nresults < 0 {
            if (lua_state.get_call_info(caller).top as usize) < new_top {
                lua_state.get_call_info_mut(caller).top = new_top as u32;
            }
        } else {
            let caller_top = lua_state.get_call_info(caller).top as usize;
            lua_state.set_top_raw(caller_top);
        }
    }
    Ok(())
}
...
fn finish_c_frame(lua_state: &mut LuaState, ci: &mut CallInfo) -> LuaResult<()> {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    if ci.c_k_function != 0 && !has_recst {
        return finish_c_api_continuation(lua_state, ci);
    }
    if call_status & CIST_YPCALL != 0 {
        if has_recst {
            ...
            let close_level = if ci.c_k_function != 0 {
                (ci.c_k_func_index as usize).saturating_add(1)
            } else {
                pcall_func_pos + 1
            };
            ...
            match close_result {
                Ok(()) => {
                    ...
                    if ci.c_k_function != 0 {
                        ...
                        lua_state.stack_set(result_pos, result_err)?;
                        lua_state.set_top_raw(result_pos + 1);
                        ci.c_k_status = 2; // LUA_ERRRUN
            ...
            match op {
                ...
                | OpCode::Eq => {
                    ...
                    if top > 0 {
                        ...
                        let mut res = !res_val.is_nil() && !(res_val == LuaValue::boolean(false));
                        if pending_finish == -3 {
                            res = !res;
                        }
                        lua_state.set_top_raw(top - 1);
                ...
                OpCode::Concat => {
                    // The pending value stores the logical concat top. Each yielded
                    // pair reduces that top by one before the next pair is attempted.
                    let top = lua_state.get_top();
                    if top > 0 {
                        let a = interrupted_instr.get_a() as usize;
                        let concat_start = base_tmp + a;
                        let concat_top = if pending_finish >= 0 {
                            pending_finish as usize
                        } else {
                            concat_start + interrupted_instr.get_b() as usize
                        };
                        let result = lua_state.stack_mut()[top - 1];
                        lua_state.stack_mut()[concat_top - 2] = result;
                        let total = concat_top - 1 - concat_start;
                        if total > 1 {
                            let next_top = concat_top - 1;
                            lua_state.set_top_raw(next_top);
                            match concat(lua_state, Some(ci), total) {
                                Ok(()) => {}
...
pub(crate) fn for_tonumber(v: StkId, out: &mut f64) -> bool {
    if v.is_float() {
        *out = v.fltvalue();
        true
