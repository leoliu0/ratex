...
pub enum OpCode {
    ...
    // Lua 5.3 compatibility opcodes. They retain the 5.3 register layouts so
    // standard binary chunks can be loaded and dumped without shadow state.
    ForLoop53,   // A Bx: increment R[A], publish R[A+3], jump back
    ForPrep53,   // A Bx: normalize numeric controls and jump forward
    TForCall53,  // A C: call iterator using control in R[A+2]
    TForLoop53,  // A Bx: publish next control and jump back
...
impl OpCode {
    ...
    pub fn get_mode(self) -> OpMode {
        ...
        match self {
            ...
            | ForLoop53 | ForPrep53 | TForLoop53 => OpMode::IABx,
