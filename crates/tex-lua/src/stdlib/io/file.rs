...
impl LuaFile {
    ...
    /// Read a line as raw bytes. Lua strings are byte sequences, so file
    /// decoding must never depend on the host's UTF-8 rules.
    fn read_line_bytes(&mut self, keep_newline: bool) -> io::Result<Option<Vec<u8>>> {
        #[cfg(not(target_arch = "wasm32"))]
        if let FileInner::PopenRead(reader) = &mut self.inner {
            return reader.read_line(keep_newline);
        }

        let mut line = Vec::new();
        let n = match &mut self.inner {
            FileInner::Read(reader) => reader.read_until(b'\n', &mut line)?,
            FileInner::ReadWrite(reader) => reader.read_until(b'\n', &mut line)?,
            FileInner::Stdin => io::stdin().lock().read_until(b'\n', &mut line)?,
            _ => return Err(io::Error::other("File not opened for reading")),
        };
        if n == 0 {
            return Ok(None);
        }
        if !keep_newline && line.last() == Some(&b'\n') {
            line.pop();
    ...
    pub fn read_line(&mut self) -> io::Result<Option<Vec<u8>>> {
        self.read_line_bytes(false)
    }

    /// Read a line, keeping the trailing newline.
    pub fn read_line_with_newline(&mut self) -> io::Result<Option<Vec<u8>>> {
        self.read_line_bytes(true)
    }
