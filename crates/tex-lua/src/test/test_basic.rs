...
#[test]
fn standard_lua53_chunks_load_execute_and_dump_roundtrip() {
    let mut vm =
        GlobalState::new_with_language(SafeOption::default(), crate::LuaLanguageLevel::Lua53);
    vm.open_stdlib(crate::stdlib::Stdlib::All).unwrap();
    let chunk = vm
        .create_binary(include_bytes!("test_data/official53_nested.luac").to_vec())
        .unwrap();
    vm.set_global("official_chunk", chunk).unwrap();
    let ops = vm
        .create_binary(include_bytes!("test_data/official53_ops.luac").to_vec())
        .unwrap();
    vm.set_global("official_ops", ops).unwrap();
    let result = vm.main_state().execute(
        r#"
        local actual = assert(load(official_chunk, '@probe', 'b'))()
        assert(actual == 684, tostring(actual))
        assert(assert(load(official_ops, '@ops', 'b'))() == "ab10")
        local ops_function = assert(load(official_ops, '@ops-dump', 'b'))
        dumped_ops = string.dump(ops_function)
        assert(assert(load(dumped_ops, '@ops-roundtrip', 'b'))() == "ab10")
        local function exported(n)
            local total = 0
            for i = 1, n do total = total + i end
            for k, value in ipairs({2, 3}) do total = total + k + value end
            local function finish(extra) return total + extra end
            return finish(#"a\0b")
        end
        dumped53 = string.dump(exported)
        assert(assert(load(dumped53, '@roundtrip', 'b'))(3) == 17)
        "#,
    );
    if let Err(error) = result {
        let full = vm.main_state().get_full_error(error);
        panic!("{}", full.message());
    }
}
