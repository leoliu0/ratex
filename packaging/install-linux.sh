...
#
# Installs ratex plus the runtime texmf overlay.
...
Install ratex and its runtime texmf overlay on Linux.
...
resolve_payload() {
    ...
    if [ ! -x "$_target/ratex" ] && [ ! -x "$_target/texmk" ] && [ ! -x "$_target/pdflatex" ]; then
