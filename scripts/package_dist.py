...
def generate_lsr(root: Path) -> None:
    """Generate a standard TeX ls-R database index for fast O(1) directory lookups."""
    lines = ["% ls-R -- filename database generated by ratex packaging", ""]
...
def stage_assets(texmf_stage: Path, full: bool = False) -> dict:
    ...
    # Unpack the authoritative bundled package and font collection
    packages_tar_zst = REPO / "crates" / "tex-kpse" / "assets" / "packages.tar.zst"
    if packages_tar_zst.is_file():
        print(f"    unpacking bundled TeX assets from {packages_tar_zst.name}...")
        cmd = ["tar", "--zstd", "-xf", str(packages_tar_zst), "-C", str(texmf_stage)]
        subprocess.run(cmd, check=True)
        placed["packages.tar.zst"] = "bundled-archive"
    if full:
        dist = None
        for r in roots:
